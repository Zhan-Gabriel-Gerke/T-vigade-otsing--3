...
def vahetus(a:int,b:int):
    """Kui min suurem kui max siis vahetamine neid omevahel
    :param int a: minimaalne arv, mis on suurem kui max
    :param int b: maximalne arv, mis on väikesem kui min
    :trype: int, int
    """
    abi=a
    a=b
...
def generator(n:int,loend:list,a:int,b:int):
    """создаёт рандомное число и добовляет их в список
    """
    for i in range(n):
        loend.append(randint(a,b))
def jagamine(loend:list,p:list,n:list,nol:list):
    """сортируем на 3 списка больше меньше 0 и остальные
    """
    for el in loend:
        if el>0:
            p(append(el))
            ...
            nol(append(el))
def keskmine(loend):
    """Показывает среднее отрицательное число и средне положительное
    """
    n=len(loend)
    if n==0:
        kesk=0
